...
def merge(output_file_path: str, merged_file_paths: Sequence[str], append_linesep: bool = True) -> None:  # noqa: FBT001
    with Path.open(output_file_path, "w") as output_file:
        for merged_file_path in merged_file_paths:
            with Path.open(merged_file_path) as merged_file:
...
def test_merge(tmpdir_factory, merged_file_count: int, append_linesep: bool):  # noqa: FBT001
    ...
    for i in range(merged_file_count):
        ...
        with Path.open(merged_file_path, "w+") as file:
    ...
    with Path.open(output_file_path) as output_file:
